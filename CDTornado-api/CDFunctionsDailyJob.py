...
def CalYield(code,Pv):
    try:
        ...
        Df = pd.read_sql(
            ...
            "select * from openquery(WIND,'select ceil(to_date(B_INFO_MATURITYDATE,''yyyyMMdd'') - sysdate) D from CBondDescription  where S_INFO_WINDCODE = ''" + code + "'' ')",
            Engine)
        D = float(Df.ix[0, 0])
        TY = 365
        Fv = 100
...
Position = getAllCDPositons()
Position = CalFundCost(Position,0.038)
Position['COSTYIELD'] = Position.apply(lambda x:CalYield(x['BONDCODE'],x['FundCostFullPrice']),axis=1)
