...
# 读取曲线估值
def GetCurve(bondtype):
    maxDate = pd.read_sql("select * from openquery(WINDNEW,'select max(trade_days)  from CBondCalendar t  where S_INFO_EXCHMARKET = ''NIB''  and trade_days < to_char(sysdate,''yyyymmdd'') ')",Engine).ix[0,0]
    DfBond = pd.read_sql("select * from openquery(WINDNEW,'select  b.s_info_windcode code,B_ANAL_MATU_CNBD bondterm,B_ANAL_YIELD_CNBD bondyield  from CBONDANALYSISCNBD1 a inner join CBondDescription b on  a.s_info_windcode = b.s_info_windcode  where B_ANAL_MATU_CNBD < 10  and a.trade_dt = ''"+maxDate+"''  and b.s_info_name like ''%"+bondtype+"%'' and b.s_info_exchmarket = ''NIB'' and b.B_ISSUE_FIRSTISSUE>=''20170101''   and length(b.s_info_windcode) =9    and   b_anal_credibility=''推荐''   and  b_info_subordinateornot = 0  order by B_ANAL_MATU_CNBD')",Engine)
    DfCurve = pd.read_sql("select * from openquery(WINDNEW,'select t.b_anal_curveterm curveterm,t.b_anal_yield curveyield from CBondCurveCNBD t  where  trade_dt = ''"+maxDate+"''  and B_ANAL_CURVETYPE = 2  and B_ANAL_CURVETERM <= 10  and B_ANAL_CURVENAME  like ''%"+bondtype+"%''  order by B_ANAL_CURVETERM')",Engine)
