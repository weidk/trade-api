...
def ReadRelativePrice(startday):
    ...
    r.set('ralativeprice_' + startday + '_' + DH.today2str(), rstData)
...
def ReadCBRelativePrice(startday):
    ...
    r.set('ralativeprice_' + startday + '_' + DH.today2str(), json.dumps({
        '排序': RankDf.to_dict(orient='records'),
        'all': ReletiveDf.to_dict(orient='records'),
    }))
...
def ReadAbsolutePrice(startday, endday):
    Df = pd.read_sql(
        "select  * from VarietiesYield where 日期>='" + startday + "'     and 日期<='" + endday + "'   order by 日期",
        EngineIS)
...
def CurveMatBag():
    ...
    startday = dt.replace(year=dt.year - 3).strftime('%Y-%m-%d')
    ReadRelativePrice(startday)
