...
def FindTradeDay(day):
    ...
    if dayValue == None:
        dayDf = pd.read_sql(
            "select * from openquery(WINDNEW,'select  max(t.trade_dt) td from CBondCurveMembersCNBD t where trade_dt<=''" + day + "'' ')",
            Engine)
...
def QueryHidenCreditHistory(code):
    HistoryDf = pd.read_sql(
        "select * from openquery(WINDNEW,'select b.s_info_windcode code, a.tddate,  a.cnbd_creditrating HIDENCREDIT,  b.B_ANAL_YIELD_CNBD EYIELD from (select min(t.trade_dt) tddate, t.cnbd_creditrating  from CBondCurveMembersCNBD t  where t.s_info_windcode = ''" + code + "''  and length(t.cnbd_creditrating) > 0  group by t.cnbd_creditrating  order by tddate) a inner join CBondAnalysisCNBD  b  on a.tddate = b.trade_dt where b.s_info_windcode = ''" + code + "''   and b.B_ANAL_CREDIBILITY=''推荐'' ')",
        Engine)
